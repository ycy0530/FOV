...
## Email results + lead notifications (V2)

The results page lets users email their result to themselves. Setup (one time, ~10 min):

1. Sign up at [resend.com](https://resend.com) (free tier: 100 emails/day)
2. Resend → Domains → Add `norrisyumortgage.com` → add the DNS records it shows at your registrar → wait for "Verified"
3. Resend → API Keys → create a key
4. Vercel → Project → Settings → Environment Variables, add:
   - `RESEND_API_KEY` = your key
   - `EMAIL_FROM` = `FOV <results@norrisyumortgage.com>`
   - `NOTIFY_EMAIL` = `norris.yu@elitelending.ca` (already the default)
5. Redeploy

**CASL/PIPEDA design:** the user's email is used once to send their result and never stored. Norris receives a completion notification on every quiz; the user's email address is included **only if** they tick the optional follow-up consent checkbox. Until Resend is configured, the email form shows a friendly "not configured" message — everything else works.
